...
public class StateDisplay {
	...
	public void render(Sprite sprite){
		...
		this.sprite = sprite;
		box.getChildren().clear();
		sprite.getComponent(Cooldown.TYPE).ifPresent((cooldown) -> {
			sprite.getComponent(SkillSet.TYPE).ifPresent((skillset) -> {
				if (skillset.getSkill(FireProjectileSkill.TYPE) != null) {
					makeSingleStat("Projectile reloading... ", Math.round(cooldown.getTimeRemaining(FireProjectileSkill.TYPE)), sprite);
				}
			});
		});
